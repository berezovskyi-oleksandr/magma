...
func (rpadde RPAddressElement) MarshalBinary() ([]byte, error) {
	if rpadde.Length == 0x0 {
		return []byte{rpadde.Length}, nil
	}

	if int(rpadde.Length) - 1 != len(rpadde.Number) {
...
func (rpce *RPCauseElement) UnmarshalBinary(input []byte) int {
	rpce.Length = input[0]
	rpce.Cause = input[1]
	if int(rpce.Length) == 2 {
...
func (rpm RPMessage) MarshalBinary() ([]byte, error) {
	...
	switch rpmt {
	case RP_DATA:
		oa_data, err := rpm.OriginatorAddress.MarshalBinary()
		if err != nil {
			return nil, err
		}
		da_data, err := rpm.DestinationAddress.MarshalBinary()
		if err != nil {
			return nil, err
		}
		b = append(b, oa_data...)
		...
		b = append(b, ud_data...)
	case RP_ERROR:
		cause_data := rpm.Cause.MarshalBinary()
...
func (rpm *RPMessage) UnmarshalBinary(input []byte) error {
	...
	case RP_ERROR:
		...
		idx += rpm.Cause.UnmarshalBinary(input[idx:])
