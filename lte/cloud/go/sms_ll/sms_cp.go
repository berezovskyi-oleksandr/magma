...
func CreateCPMessage(txID byte, messageType byte, rpdu []byte) (CPMessage, error) {
	cpm := new(CPMessage)
	err := cpm.setTransactionId(txID)
	if err != nil {
		return *cpm, err
	}
	err = cpm.setProtocolDisc(CP_PROTOCOL_DISC)
	if err != nil {
		return *cpm, err
	}

	switch messageType {
	case CP_DATA, CP_ERROR, CP_ACK:
		cpm.MessageType = messageType
	default:
		return *cpm, SMSCPError("Invalid CP MessageType")
	}

	cpm.Length = byte(len(rpdu))
	cpm.RPDU = make([]byte, len(rpdu))
	copy(cpm.RPDU, rpdu)
	return *cpm, nil
}
...
func (cpm CPMessage) MarshalBinary() ([]byte, error) {
	...
	default:
		return nil, SMSCPError("Invalid CP MessageType")
	}
