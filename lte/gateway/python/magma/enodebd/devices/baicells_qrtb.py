...
from magma.enodebd.state_machines.enb_acs_states import EnodebAcsState, \
    ...
    BaicellsSendRebootState, WaitRebootResponseState, CheckOptionalParamsState, \
...
class BaicellsQRTBHandler(BasicEnodebAcsStateMachine):
    ...
    def _init_state_map(self) -> None:
        self._state_map = {
            'wait_inform': WaitInformState(self,
                when_done='wait_empty', when_boot=None),
            'wait_empty': WaitEmptyMessageState(self,
                when_done='get_transient_params',
                when_missing='check_optional_params'),
            'check_optional_params': CheckOptionalParamsState(self,
                when_done='get_transient_params'),
            'get_transient_params': SendGetTransientParametersState(self,
                when_done='wait_get_transient_params'),
            'wait_get_transient_params': WaitGetTransientParametersState(self,
                when_get='get_params', when_get_obj_params='get_obj_params',
                when_delete='delete_objs', when_add='add_objs',
                when_set='set_params', when_skip='end_session'),
            'get_params': GetParametersState(self,
                when_done='wait_get_params'),
            'wait_get_params': WaitGetParametersState(self,
                when_done='get_obj_params'),
            'get_obj_params': GetObjectParametersState(self,
                when_done='wait_get_obj_params'),
            'wait_get_obj_params': WaitGetObjectParametersState(self,
                when_delete='delete_objs', when_add='add_objs',
                when_set='set_params', when_skip='end_session'),
            'delete_objs': DeleteObjectsState(self, when_add='add_objs',
                when_skip='set_params'),
            'add_objs': AddObjectsState(self, when_done='set_params'),
            'set_params': SetParameterValuesState(self,
                when_done='wait_set_params'),
            'wait_set_params': WaitSetParameterValuesState(self,
                when_done='check_get_params', when_apply_invasive='reboot'),
            'check_get_params': GetParametersState(self,
                when_done='check_wait_get_params', request_all_params=True),
            'check_wait_get_params': WaitGetParametersState(self,
                when_done='end_session'), 'end_session': EndSessionState(self),
            'reboot': BaicellsSendRebootState(self, when_done='wait_reboot'),
            'wait_reboot': WaitRebootResponseState(self,
                when_done='wait_inform_post_reboot'),
            'wait_inform_post_reboot': WaitInformState(self,
                when_done='wait_empty_post_reboot', when_boot=None),
            'wait_empty_post_reboot': WaitEmptyMessageState(self,
                when_done='get_transient_params',
                when_missing='check_optional_params'),
...
class BaicellsQRTBTrDataModel(DataModel):
    ...
    PARAMETERS = {  # Top-level objects
        ParameterName.DEVICE: TrParam(DEVICE_PATH, True,
                                      TrParameterType.OBJECT, False),
        ParameterName.FAP_SERVICE: TrParam(FAPSERVICE_PATH, True,
                                           TrParameterType.OBJECT, False),

        # Device info parameters
        ParameterName.GPS_STATUS: TrParam(
            DEVICE_PATH + 'DeviceInfo.X_COM_GPS_Status', True,
            TrParameterType.BOOLEAN, False),
        ParameterName.PTP_STATUS: TrParam(
            DEVICE_PATH + 'DeviceInfo.X_COM_1588_Status', True,
            TrParameterType.BOOLEAN, False),
        ParameterName.MME_STATUS: TrParam(
            DEVICE_PATH + 'DeviceInfo.X_COM_MME_Status', True,
            TrParameterType.BOOLEAN, False),
        ParameterName.REM_STATUS: TrParam(
            DEVICE_PATH + 'DeviceInfo.X_COM_REM_Status', True,
            TrParameterType.BOOLEAN, False),
        ParameterName.LOCAL_GATEWAY_ENABLE: TrParam(
            DEVICE_PATH + 'DeviceInfo.X_COM_LTE_LGW_Switch', False,
            TrParameterType.BOOLEAN, False),
        ParameterName.GPS_ENABLE: TrParam(
            DEVICE_PATH + 'DeviceInfo.X_COM_GpsSyncEnable', False,
            TrParameterType.BOOLEAN, False),
        ParameterName.GPS_LAT: TrParam(
            DEVICE_PATH + 'FAP.GPS.LockedLatitude', True,
            TrParameterType.INT, False),
        ParameterName.GPS_LONG: TrParam(
            DEVICE_PATH + 'FAP.GPS.LockedLongitude', True,
            TrParameterType.INT, False),
        ParameterName.SW_VERSION: TrParam(
            DEVICE_PATH + 'DeviceInfo.SoftwareVersion', True,
            TrParameterType.STRING, False),
        ParameterName.SERIAL_NUMBER: TrParam(
            DEVICE_PATH + 'DeviceInfo.SerialNumber', True,
            TrParameterType.STRING, False),

        # Capabilities
        ParameterName.DUPLEX_MODE_CAPABILITY: TrParam(
            FAPSERVICE_PATH + 'Capabilities.LTE.DuplexMode', True,
            TrParameterType.STRING, False),
        ...
        ParameterName.EARFCNDL: TrParam(
            FAPSERVICE_PATH + 'CellConfig.LTE.RAN.RF.EARFCNDL', True,
            TrParameterType.INT, False),
        ParameterName.EARFCNUL: TrParam(
            FAPSERVICE_PATH + 'CellConfig.LTE.RAN.RF.EARFCNUL', True,
            TrParameterType.INT, False),
        ParameterName.BAND: TrParam(
            FAPSERVICE_PATH + 'CellConfig.LTE.RAN.RF.FreqBandIndicator', True,
            TrParameterType.INT, False),
        ParameterName.PCI: TrParam(
            FAPSERVICE_PATH + 'CellConfig.LTE.RAN.RF.PhyCellID', False,
            TrParameterType.INT, False),
        ...
        ParameterName.OP_STATE: TrParam(
            FAPSERVICE_PATH + 'FAPControl.LTE.OpState', True,
            TrParameterType.BOOLEAN, False),
        ...
        ParameterName.CELL_RESERVED: TrParam(
            FAPSERVICE_PATH + 'CellConfig.LTE.RAN.CellRestriction.CellReservedForOperatorUse',
            True, TrParameterType.BOOLEAN, False),

        # Core network parameters
        ParameterName.MME_IP: TrParam(
            FAPSERVICE_PATH + 'FAPControl.LTE.Gateway.S1SigLinkServerList',
            True, TrParameterType.STRING, False),
        ...
        ParameterName.PLMN: TrParam(
            FAPSERVICE_PATH + 'CellConfig.LTE.EPC.PLMNList.', True,
            TrParameterType.STRING, False),  # PLMN arrays are added below
        ParameterName.TAC: TrParam(
            FAPSERVICE_PATH + 'CellConfig.LTE.EPC.TAC', True,
            TrParameterType.INT, False),
        ParameterName.IP_SEC_ENABLE: TrParam(
            DEVICE_PATH + 'Services.FAPService.Ipsec.IPSEC_ENABLE', False,
            TrParameterType.BOOLEAN, False),
        ParameterName.MME_POOL_ENABLE: TrParam(
            FAPSERVICE_PATH + 'FAPControl.LTE.Gateway.X_COM_MmePool.Enable',
            True, TrParameterType.BOOLEAN, False),
        ...
        ParameterName.PERF_MGMT_ENABLE: TrParam(
            DEVICE_PATH + 'FAP.PerfMgmt.Config.1.Enable', False,
            TrParameterType.BOOLEAN, False),
        ...
        ParameterName.PERF_MGMT_UPLOAD_URL: TrParam(
            DEVICE_PATH + 'FAP.PerfMgmt.Config.1.URL', False,
            TrParameterType.STRING, False),
    ...
    for i in range(1, NUM_PLMNS_IN_CONFIG + 1):
        # We need to handle Baicells bug which inverts the meaning of
        # 'cell reserved'
        TRANSFORMS_FOR_ENB[ParameterName.PLMN_N_CELL_RESERVED % i] = \
            transform_for_enb.invert_cell_reserved_boolean
        TRANSFORMS_FOR_MAGMA[ParameterName.PLMN_N_CELL_RESERVED % i] = \
            transform_for_enb.invert_cell_reserved_boolean

        PARAMETERS[(ParameterName.PLMN_N) % i] = TrParam(
            FAPSERVICE_PATH + 'CellConfig.LTE.EPC.PLMNList.%d.' % i, True,
            TrParameterType.STRING, False)
    ...
    @classmethod
    def _get_magma_transforms(cls, ) -> Dict[
        ParameterName, Callable[[Any], Any]]:
    ...
    def get_parameter_names(cls) -> List[ParameterName]:
        ...
        names = list(filter(lambda x: (not str(x).startswith('PLMN')) and (
                str(x) not in excluded_params), cls.PARAMETERS.keys()))
        return names

    @classmethod
    def get_numbered_param_names(cls) -> Dict[
